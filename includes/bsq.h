...
/*   Updated: 2026/05/21 22:37:58 by idilsincer       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*Read buffer size for slurp(). 
Each read() call ingests at most this many bytes 
before the chunk is appended to the accumulation buffer. 
*/
...
typedef struct s_canvas
{
	...
	int		rows;
	int		cols;
	char	vacant;		// Characters that marks a free cell
	char	blocked;	// Characters that marks an obstacle
	char	marked;		// Characters used to paint the solution square
}	t_canvas;

typedef struct s_square
{
	int	side;			// Side length (0 means no solution was found)
	int	row;
	int	col;
}	t_square;
